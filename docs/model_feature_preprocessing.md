...
| `kapt_max_floor`, `floor` | `kapt_max_floor`, `kapt_relative_floor`, `kapt_relative_floor_bin`, `floors_below_kapt_top`, `floors_below_kapt_top_bin` | KAPT 단지 기본정보의 최고층은 기존 거래기반 상대층수를 대체하지 않고 보조 feature로 추가합니다. KAPT 최고층이 결측이거나 거래층보다 낮으면 `kapt_max_floor_missing = 1`로 처리합니다. |
